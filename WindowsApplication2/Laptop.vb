Public Class Laptop
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        MessageBox.Show("Confirm by checking the LED on the charging adapter, if not PRESS OK", "Checking1", MessageBoxButtons.OK, MessageBoxIcon.Information)
        MessageBox.Show("Confirm if the adapter is working by removing the battery and using only the adapter to Turn ON the system, Or check the voltage output using a multimeter", "Troubleshooting", MessageBoxButtons.OK, MessageBoxIcon.Information)
    End Sub

    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        MessageBox.Show("Unplug the charging adapter and disconnect the battery, remove the required casing to access the RAM with your screwdriver, Gently remove the RAM while holding its edges, avoid toching the circuitory or chips, then clean the lower edge that is has gold color using a cotton dipped in petrol or spirit, and fix it back", "Troubleshooting1", MessageBoxButtons.OK, MessageBoxIcon.Information)
